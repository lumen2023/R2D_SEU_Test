...
class RiskFieldCalculator:
    ...
    DEFAULTS = {
        ...
        "risk_field_boundary_weight": 0.0,            # 道路边界权重
        "risk_field_lane_weight": 0.01,                # 车道线权重
        "risk_field_offroad_weight": 0.0,             # 偏离道路权重
        "risk_field_vehicle_weight": 1.0,             # 周围车辆权重
        "risk_field_object_weight": 0.0,              # 静态障碍物权重
        "risk_field_headway_weight": 0.0,             # 车头时距权重 0
        "risk_field_ttc_weight": 0.0,                 # 碰撞时间权重 0
        "risk_field_boundary_sigma": 0.75,            # 边界风险扩散系数（米）
        "risk_field_lane_edge_sigma": 0.75,           # 车道边缘风险扩散系数（米）
        "risk_field_broken_line_factor": 0.05,        # 虚线换道线风险系数（允许跨越，低风险）
        "risk_field_solid_line_factor": 0.60,         # 普通实线风险系数（不鼓励跨越）
        ...
        "risk_field_vehicle_longitudinal_sigma": 5.0, # 车辆纵向风险扩散（米）
        "risk_field_vehicle_lateral_sigma": 1.6,      # 车辆横向风险扩散（米）
        "risk_field_vehicle_beta": 2.0,               # 车辆超高斯指数（控制形状）
        "risk_field_vehicle_dynamic_sigma_scale": 2.0,# 动态风险sigma缩放
        "risk_field_vehicle_dynamic_alpha": 0.9,      # 动态风险不对称系数
        "risk_field_vehicle_min_dynamic_sigma": 0.5,  # 最小动态sigma
        "risk_field_object_longitudinal_sigma": 5.0,  # 障碍物纵向风险扩散（米）
        "risk_field_object_lateral_sigma": 1.6,       # 障碍物横向风险扩散（米）
        ...
        "risk_field_headway_cost_clip": 3.0,          # 车头时距成本上限
    ...
    def _road_risk(self, env: Any, vehicle: Any, lane: Any) -> Tuple[float, Dict[str, Any]]:
        ...
        if lane is not None:
            ...
            if math.isfinite(lateral_offset) and math.isfinite(lane_width) and lane_width > self.EPS:
                ...
                lane_cost = max(
                    side0_line_profile["factor"] * self._one_dimensional_risk(side_0_gap, side0_line_profile["sigma"]),
                    side1_line_profile["factor"] * self._one_dimensional_risk(side_1_gap, side1_line_profile["sigma"]),
                )
    ...
    def lane_line_risk_profile(self, lane: Any, side: int) -> Dict[str, Any]:
        ...
        sigma_key = "risk_field_boundary_sigma" if kind == "boundary" else "risk_field_lane_edge_sigma"
        factor_key = {
    ...
    @staticmethod
    def _two_dimensional_risk(long_gap: float, lat_gap: float, sigma_long: float, sigma_lat: float) -> float:
        """计算二维高斯风险
        
        公式：exp(-(long_gap²/(2*sigma_long²) + lat_gap²/(2*sigma_lat²)))
        ...
        exponent = -(
            (max(long_gap, 0.0) ** 2) / (2.0 * sigma_long ** 2)
            + (max(lat_gap, 0.0) ** 2) / (2.0 * sigma_lat ** 2)
        )
